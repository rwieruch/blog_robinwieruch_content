...
description = "Basics about React's data fetching to interact with real world data by using the native fetch API..."
...
summary = "Newcomers to React often start with applications that don't need data fetching at all. Usually they are confronted with Counter, Todo or TicTacToe applications. That's good, because data fetching adds another layer of complexity to your application while taking the first steps in React. However, at some point you want to interact with real world data from an own or a third-party API."
+++
...
However, at some point you want to interact with real world data from an own or a third-party [API](https://www.robinwieruch.de/what-is-an-api-javascript/). The article gives you a walkthrough on how to fetch data in plain React. There is no external state management solution involved to store your fetched data. It uses only React's local state management.

{{% chapter_header "Where to fetch in React's component tree?" "react-where-fetch" %}}

Imagine you already have a component tree that has several levels in its hierarchy. Now you are about to fetch a list of items from a third-party API. Which level in your component hierarchy, so which specific component, should fetch the data? Basically it depends on three criteria:

* 1. Which components are interested in this data? The fetching component should be a common parent component for all these components.
...
* 2.2. When the loading indicator should be shown in child components of the common parent component, not necessarily the components that need the data, the common component would still be the component to fetch the data. The loading indicator state would then be passed down to all child components that would be interested to show a loading indicator.
...
Even though the `render()` method already ran once before the `componentDidMount()` method, you don't run into any null pointer exception because you have initialized the `hits` property in the local state with an empty array.

{{% chapter_header "What about a loading spinner and error handling?" "react-fetch-loading-error" %}}
...
A loading indicator can be as simple as a Loading... message, but you can also use third-party libraries to show a spinner or {{% a_blank "pending content component" "https://github.com/danilowoz/react-content-loader" %}}. It is up to you to signalize your end-user that data is being fetched.

The second state you could keep in your local state would be an error state. When an error occurs in your application, nothing is worse than giving your end-user no indication about the error.
...
The shown way to fetch data can be repetitive in several applications. Once a component mounted, you want to fetch data and show conditional loading or error indicators. The component so far can be split up into two responsibilities: showing the fetched data with conditional renderings and fetching the state with local state management. The latter could be made reusable by a [higher order component](https://www.robinwieruch.de/gentle-introduction-higher-order-components/). (If you are going to read the linked article, you will also see how you could abstract away the conditional renderings in HOCS. After that your component would only be concerned to display the fetched data.)
...
The higher order component receives an url that will be used to `fetch()` the data. The url that is passed later on will be the specific `API + DEFAULT_QUERY` as you have used before. If you need to pass more query parameters to your higher order component later on, you could extend the arguments.
...
In addition, the higher order component uses a generic data container in the local state called `data`. It is not aware anymore of the specific naming as before.

In the second step, you can dispose all of the fetching and state logic from your `App` component. Because it has no local state or lifecycle methods anymore, you can refactor it to a functional stateless component.
...
Basically that's it to abstract away the data fetching in React. By using higher order components to fetch the data, you can easily opt-in this feature for any component with any endpoint API url.
