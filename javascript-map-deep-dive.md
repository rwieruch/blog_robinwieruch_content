...
{{% chapter_header "What is the Array Map Method in JavaScript?" "what-is-the-map-method" %}}
...
myArray.map((value, index, arr) =>
  arr[index - 1] ? value + arr[index - 1] : value,
);

// Just using the variable that holds the original array.
myArray.map((value, index) =>
  myArray[index - 1] ? value + myArray[index - 1] : value,
);
{{< /highlight >}}
...
{{< highlight javascript >}}
const transformedObject = mapped.reduce(
...
const mapped = arr.map(value => value.map(number => number * 2));

console.log(mapped); // [[2, 4, 6], [8, 10, 12], [14, 16, 18]]
{{< /highlight >}}
